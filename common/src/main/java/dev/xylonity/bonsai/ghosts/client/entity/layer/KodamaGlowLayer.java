...
public class KodamaGlowLayer extends GeoRenderLayer<KodamaEntity> {
    ...
    public void render(PoseStack poseStack, KodamaEntity animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        RenderType glowRenderType = RenderType.entityTranslucent(new ResourceLocation(Ghosts.MOD_ID, "textures/entity/kodama_" + animatable.getVariant() + ".png"));
        ...
        getRenderer().reRender(bakedModel, poseStack, bufferSource, animatable, glowRenderType, glowBuffer, partialTick, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY, 1, 1, 1, alpha);
    }
